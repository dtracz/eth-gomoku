...
export class TransferComponent implements OnInit {
  ...
  private createForms() {
    this.userForm = this.fb.group({
      transferAddress: new FormControl(this.user.transferAddress, Validators.compose([Validators.required, Validators.minLength(42), Validators.maxLength(42)])),
  ...
  submitForm() {
    if (this.userForm.invalid) {
      ...
      this.transferService.transferEther(this.userForm.value).then(() => {
      }).catch(error => {
        console.log(error);
      });
    }
